...
to setup
  ...
  [set  pcolor green]
  ask patches with [abs pxcor = 16 or abs pycor = 16] [set pcolor red]
end

 to go
  LET Still-Going true

  ask turtles with [who = 0] [fd 1
   if pcolor = red or pcolor = magenta [set Still-Going false]

    if pcolor = green AND score < 5
    [bait
      set score (score + 1)
      ask one-of patches with [distance turtle 0 > 5 AND pcolor = black]
      [set pcolor green]]
  ...
  [set TTL (TTL - 1)
    If TTL = 0 [set pcolor 0]]
  ]
    If still-going = false [finish-game stop]
  wait 1 / speed
end

to bait
  let tumblr true
  if pcolor != black [set tumblr false]
  if tumblr = false [ask patches with [pcolor = magenta] [set pcolor black]]
   ask n-of (score + 2) patches with [distance Turtle 0 > 5 AND
      pcolor != red AND
      pcolor != green]
      [set pcolor magenta]
end
...
to tryhard
  let alexa true
  if pcolor = green [set alexa false]
  if alexa = false [ask patches with [pcolor = magenta] [set pcolor black]]
  if alexa = false [ask turtles with [who >= 1] [die]]
  ask one-of patches with [pcolor = black AND abs pxcor < 14 AND abs pycor < 14] [sprout 1]
  ask turtles with [who >= 1] [set color yellow]
end

to tryhard2
  Ask turtles with [who >= 1] [
    set heading random 3 * 90 + 90
    set color yellow
    set pcolor magenta
    fd 1
    set TTL 4
  if abs pxcor >= 14 or abs pycor >= 14 or pcolor = green
    [die]]
end
